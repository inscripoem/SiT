...
def train_SiT_pretrain(args):
    ...
    # Resume training 
    to_restore = {"epoch": 0, "lr_schedule": [], "wd_schedule": [], "momentum_schedule": []}
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for it, ((clean_crops, corrupted_crops, masks_crops), _) in enumerate(metric_logger.log_every(data_loader, header)):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                if plot_==True:
                    ...
                    writter.add_image('reconstruction_samples', torchvision.utils.make_grid(imagesToPrint, nrow=min(15, bz), normalize=True, range=(-1, 1)), epoch)
            ...
            # save to tensorboard
            writter.add_scalar('Loss/c_loss', c_loss.item(), it)
            writter.add_scalar('Loss/r_loss', r_loss.item(), it)
            writter.add_scalar('Loss/total_loss', loss.item(), it)
            writter.add_scalar('Accuracy/top1_c_acc', top1, it)
            writter.add_scalar('Accuracy/top5_c_acc', top5, it)
            writter.add_scalar('PSNR', psnr, it)
            writter.add_scalar('SSIM', ssim, it)
            writter.add_scalar('Learning Rate', optimizer.param_groups[0]["lr"], it)
            writter.add_scalar('Weight Decay', optimizer.param_groups[0]["weight_decay"], it)
